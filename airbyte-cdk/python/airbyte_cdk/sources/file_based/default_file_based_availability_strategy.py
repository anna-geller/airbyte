...
from airbyte_cdk.sources.file_based.exceptions import ConnectionCheckError, FileBasedSourceError
...
class DefaultFileBasedAvailabilityStrategy(AvailabilityStrategy):
    ...
    def _check_list_files(self, stream: AbstractFileBasedStream) -> List[RemoteFile]:
        try:
            ...
            raise ConnectionCheckError(FileBasedSourceError.ERROR_LISTING_FILES, stream=stream.name) from exc

        if not files:
            raise ConnectionCheckError(FileBasedSourceError.EMPTY_STREAM, stream=stream.name)

        if not all(f.extension_agrees_with_file_type() for f in files):
            raise ConnectionCheckError(FileBasedSourceError.EXTENSION_MISMATCH, stream=stream.name)
    ...
    def _check_parse_record(self, stream: AbstractFileBasedStream, file: RemoteFile):
        ...
        try:
            ...
            raise ConnectionCheckError(FileBasedSourceError.ERROR_READING_FILE, stream=stream.name, file=file.uri) from exc
