...
from airbyte_cdk.sources.file_based.schema_validation_policies import AbstractSchemaValidationPolicies, DefaultSchemaValidationPolicies
...
class FileBasedSource(AbstractSource, ABC):
    ...
    def __init__(
        ...
        validation_policies: AbstractSchemaValidationPolicies = DefaultSchemaValidationPolicies(),
    ):
    ...
    def check_connection(self, logger: logging.Logger, config: Mapping[str, Any]) -> Tuple[bool, Optional[Any]]:
        ...
        for stream in streams:
            try:
                ...
                errors.append(
                    f"Unable to connect to stream {stream} - {''.join(traceback.format_exception(etype=type(exc), value=exc, tb=exc.__traceback__))}"
                )
            else:
